...
const NFTCard: FC<NFTCardProps> = ({
    ...
    const handleMarkasDelivered = async () => {
        let isPayer = payer == connectedAddress;
        if (writeContractAsync) {
            try {
                await writeContractAsync(
                    {
    ...
    return (
        ...
        <div className="rounded-lg border bg-card text-card-foreground shadow-sm overflow-hidden"
            ...
            <div className=" aspect-square relative bg-muted">
                ...
                <Image
                    src={image!} width={800} height={1000} objectFit="cover" alt={name || ""}
            ...
            <div className="items-center p-4 pt-0 flex justify-between">
                ...
                {payer == connectedAddress && price && !payerChecked && payedFor && (
                    <button onClick={handleMarkasDelivered} className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&amp;_svg]:pointer-events-none [&amp;_svg]:size-4 [&amp;_svg]:shrink-0 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2">
                        Mark as Delivered ✓
                    </button>
                )}
                ...
                {payerChecked && !buyerChecked || !payerChecked && buyerChecked && (
                    <button onClick={() => { }} className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&amp;_svg]:pointer-events-none [&amp;_svg]:size-4 [&amp;_svg]:shrink-0 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2">
                        Dispute
                    </button>
                )}
                {payerChecked && buyerChecked && owner == connectedAddress && (
                    <button onClick={handleCollectPayment} className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&amp;_svg]:pointer-events-none [&amp;_svg]:size-4 [&amp;_svg]:shrink-0 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2">
                        Collect Payment
                    </button>
                )}
            </div>
